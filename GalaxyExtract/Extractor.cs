...
namespace GalaxyExtract
{
    ...
    public class Extractor : IMod
    {
        ...
        private void PauseGC()
        {
            try
            {
                gcRegionActive = GC.TryStartNoGCRegion(NO_GC_SIZE);
                if (!gcRegionActive)
                    modApi.Log("GalaxyExtractor: Could not start no-GC region; GC remains active during scan");
            }
            catch (Exception ex)
            {
                gcRegionActive = false;
